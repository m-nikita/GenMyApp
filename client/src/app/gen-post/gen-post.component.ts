...
export class GenPostComponent {
  ...
  allPost: Post[] = [];
  postGenerating: boolean = false;
  noPostMessage: boolean = true;
  postGenIndex: number = 1;
  ...
  onSubmit(f: NgForm) {
    ...
    if (!f.valid) {
      ...
      this.allPost = [];
      this.noPostMessage = false;
      this.postGenerating = true;
      this.generatePost(f);
    }
  }

  generatePost(f: NgForm) {
    ...
    this.http.post<any>(this.vGlobales.urlBack + 'openai/generate/post', body, { headers }).subscribe(data => {
      this.allPost.push({
        id: this.postGenIndex,
        text: data.answer,
        keyWords: keyWords
      });
      if (this.postGenIndex == nbPost) {
